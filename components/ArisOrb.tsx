...
export default function ArisOrb({ light = false }: { light?: boolean }) {
  ...
  useEffect(() => {
    ...
    // Outer icosahedron — dark on light, amber on dark
    const icoGeo = new THREE.IcosahedronGeometry(1.1, 1);
    const icoMat = new THREE.MeshBasicMaterial({
      color: light ? 0x1a1a1a : 0xf5a623,
      wireframe: true,
      transparent: true,
    ...
    // Inner icosahedron — always amber
    const innerGeo = new THREE.IcosahedronGeometry(0.55, 1);
    const innerMat = new THREE.MeshBasicMaterial({
      color: 0xf5a623,
      wireframe: true,
      transparent: true,
      opacity: light ? 0.45 : 0.1,
    });
    const inner = new THREE.Mesh(innerGeo, innerMat);
    scene.add(inner);

    // Particles — amber on both
  ...
  return (
    <div
      ...
      style={{
        background: light
          ? "radial-gradient(ellipse 80% 70% at 50% 50%, rgba(245,166,35,0.08) 0%, transparent 70%)"
